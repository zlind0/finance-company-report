...
> - XPU：可以把 CPU、GPU、NPU、定制 AI 加速器等各种“计算处理器”统称为 XPU。它用在 AI、HPC 和云计算系统里，作用是承担特定类型的计算负载。它大致由计算核心、片上互连、缓存或显存接口、电源管理和高速 I/O 组成。
...
> 术语解释：
> - compute offload：计算卸载，意思是把原本由 CPU 承担的一部分任务交给专用芯片去做。它用在网卡、存储卡和 AI 加速器中，作用是提升效率、降低主 CPU 负担。它大致依赖专用处理单元、DMA 数据搬运和配套驱动软件。
...
> - high-bandwidth memory：高带宽存储器，常简称 HBM。它用在高端 AI 加速器和 HPC 芯片旁边，作用是在很小面积内提供极高数据吞吐。它大致由多层堆叠 DRAM、硅中介层或先进封装和高速接口组成。
